...
class ImdbSuiteTester extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Task1") {
    assert(initializeImdb(), INIT_ERR_MSG)
    val rdd = ImdbAnalysis.titleBasicsRDD
    val expectedResult =
    ...
    assertSameElementsTask1(ImdbAnalysis.task1(rdd).collect().toList, floatedExpected)
  }

  test("Task2") {
    assert(initializeImdb(), INIT_ERR_MSG)
    val list1 = ImdbAnalysis.titleBasicsRDD
    val list2 = ImdbAnalysis.titleRatingsRDD
    val res = ImdbAnalysis.task2(list1, list2).collect().toList
  ...
  test("Task3") {
    ...
    val list1 = ImdbAnalysis.titleBasicsRDD
    val list2 = ImdbAnalysis.titleRatingsRDD
    val res = ImdbAnalysis.task3(list1, list2).collect().toList
  ...
  test("Task4") {
    assert(initializeImdb(), INIT_ERR_MSG)
    val list1 = ImdbAnalysis.titleBasicsRDD
    val list2 = ImdbAnalysis.titleCrewRDD
    val list3 = ImdbAnalysis.nameBasicsRDD
    val res = ImdbAnalysis.task4(list1, list2, list3).collect().toList
    val expectedResult = List(
