...
object Assignment9 {
  ...
  */
  ...
  class Product(override val name: String,
    ...
    //BEGIN SOLUTION
    override def toString: String = name + "," + skn + "," + qty + "," + price
  ...
  object Product {
    def apply(formattedInput: String): Product = {
      ...
      */
      //BEGIN SOLUTION
      val regex = """^\s*([\w\s]+[\w])\s*,\s*([\w\d]+)\s*,\s*(\d+)\s*,\s*(\d+.\d*)\s*""".r.pattern
      val m = regex.matcher(formattedInput)
      m.matches() match {
        case true => {
          val name = m.group(1)
          val skn = m.group(2)
          val qty = m.group(3).toInt
          val price = m.group(4).toDouble
          new Product(name, skn, qty, price)
        }
        case false => {
  ...
  class Service(override val name: String,
    ...
    override def toString: String = name + "," + service_id + "," + rate

    override def equals(p: Item): Boolean = {
      p match {
        case x: Service => {
          super.equals(x) && x.service_id == service_id
        }
        case _ => false
      }
    }
  ...
  class Ellipsoid(val center: (Double, Double, Double), val axisLengths: (Double, Double, Double)) extends Shape {
    ...
    override def translate(xShift: Double, yShift: Double, zShift: Double): Ellipsoid = {
      val newCentre = (center._1 + xShift, center._2 + yShift, center._3 + zShift)
      new Ellipsoid(newCentre, axisLengths)
    }
    //END SOLUTION
  }
  ...
  class Polyhedron(val listOfVerts: List[(Double, Double, Double)]) extends Shape with WithCorners {
    ...
    override def getCenter: (Double, Double, Double) = {
      val xsum = listOfVerts.map(_._1).sum
      val ysum = listOfVerts.map(_._2).sum
      val zsum = listOfVerts.map(_._3).sum
    ...
    override def translate(xShift: Double, yShift: Double, zShift: Double): Polyhedron = {
      val newList = listOfVerts.map(x => (x._1 + xShift, x._2 + yShift, x._3 + zShift))
      new Polyhedron(newList)
    }
  ...
  class Pyramid(x0: (Double, Double, Double), x1: (Double, Double, Double), x2: (Double, Double, Double), x3: (Double, Double, Double))
    ...
    override def translate(xShift: Double, yShift: Double, zShift: Double): Pyramid = {
      val p1 = (x0._1 + xShift, x0._2 + yShift, x0._3 + zShift)
      val p2 = (x1._1 + xShift, x1._2 + yShift, x1._3 + zShift)
      val p3 = (x2._1 + xShift, x2._2 + yShift, x2._3 + zShift)
      val p4 = (x3._1 + xShift, x3._2 + yShift, x3._3 + zShift)
      new Pyramid(p1, p2, p3, p4)
    }
  }
  ...
  class Cuboid(lowerLeft: (Double, Double, Double), length: Double, width: Double, height:Double)
    extends Polyhedron(
      List(lowerLeft,
        ...
        (lowerLeft._1, lowerLeft._2 + width, lowerLeft._3 + height))) with WithCorners {
    override def translate(xShift: Double, yShift: Double, zShift: Double): Cuboid = {
      val newLeftLower = (lowerLeft._1 + xShift, lowerLeft._2 + yShift, lowerLeft._3 + zShift)
      new Cuboid(newLeftLower, length, width, height)
    }
  }

  class Sphere(override val center: (Double, Double, Double), val rad: Double)
    extends Ellipsoid(center, (center._1 + rad, center._2 + rad, center._3 + rad)) {
    override def translate(xShift: Double, yShift: Double, zShift: Double): Sphere = {
      val newCentre = (center._1 + xShift, center._2 + yShift, center._3 + zShift)
      new Sphere(newCentre, rad)
    }
  }
  //END SOLUTION
