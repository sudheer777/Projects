...
object Assignment8 {
  ...
  case class RecordType(r: List[(String, Type)]) extends Type
    ...
    e match {
      ...
      case DeRef(e) => {
        ...
        typeOf(e, alpha) match {
          case RefType(t) => t
          case _ => throw TypeErrorException(s"Call to DeRef but with a non function type")
        }
        //END SOLUTION
      }
      ...
      case Record(ies) => {
        //BEGIN SOLUTION
        RecordType(ies.map(x => (x._1, typeOf(x._2, alpha))))
        //END SOLUTION
      }
