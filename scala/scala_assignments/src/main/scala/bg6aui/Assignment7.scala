...
object Assignment7 {
  ...
  def eval_k[T](e: Expr, env: Environment, k: Value => T): T = e match {
    ...
    case Let(id, e1, e2) => {
      val v1 = eval_k(e1, env, x => x)
      eval_k(e2, Extend(id, v1, env), k)
    }
    case Plus(e1, e2) => {
      val v1 = eval_k(e1, env, x => x)
      val v2 = eval_k(e2, env, x => x)
      addValue_k(v1, v2, k)
    }
    case Div(e1, e2) => {
      val v1 = eval_k(e1, env, x => x)
    ...
    case IfThenElse(eCond, eThen, eElse) => {
      val v1 = eval_k(eCond, env, x => x)
      v1 match {
        case BoolValue(x) => {
          if (x) eval_k(eThen, env, k) else eval_k(eElse, env, k)
        }
        case _ => throw new RunTimeError("Boolean value is expected")
      }
    }
    ...
    case FunCall(e1, e2) => {
      ...
      v1 match {
        case Closure(p, eBody, envClosure) => {
          val v2 = eval_k(e2, env, x => x)
          eval_k(eBody, Extend(p, v2, env), k)
        }
...
-- */
  ...
  /*def evalErr_k[T](e: Expr, env: Environment, k: Value => T, err_k: () => T ): T = e match {
    ...
    case Let(id, e1, e2) => {
      val v1 = evalErr_k(e1, env, x => x, () => null)
      evalErr_k(e2, Extend(id, v1, env), k, err_k)
    }
    case Plus(e1, e2) => {
      val v1 = evalErr_k(e1, env, x => x, () => null)
      val v2 = evalErr_k(e2, env, x => x, () => null)
    ...
    case IfThenElse(eCond, eThen, eElse) => {
      val v1 = evalErr_k(eCond, env, x => x, () => null)
      v1 match {
        case BoolValue(x) => {
          if (x) evalErr_k(eThen, env, k, err_k) else evalErr_k(eElse, env, k, err_k)
        }
        case _ => err_k()
      }
    }
    ...
    case FunCall(e1, e2) => {
      ...
      v1 match {
        case Closure(p, eBody, envClosure) => {
          val v2 = evalErr_k(e2, env, x => x, () => null)
          evalErr_k(eBody, Extend(p, v2, env), k, err_k)
        }
        case _ => err_k()
      }
    }
  ...
  // This is for testing. you can ignore this
