...
class Game(wall: List[(Int, Int)], coin: List[(Int, Int, Int)], initialX: Int, initialY: Int) {
   ...
   */
  def moveLeft(): Unit = {
    val newPosX = positionX - 1
    if (positionX <= 0 || field(newPosX)(positionY) == 0)
      return
  ...
  def moveRight(): Unit = {
    val newPosX = positionX + 1
    if (positionX >= 9 || field(newPosX)(positionY) == 0)
      return
   ...
   */
  def moveUp(): Unit = {
    val newPosY = positionY - 1
    if (positionY <= 0 || field(positionX)(newPosY) == 0)
      return
  ...
  def moveDown(): Unit = {
    val newPosY = positionY + 1
    if (positionY >= 9 || field(positionX)(newPosY) == 0)
      return
  ...
  def checkCoins(): Unit = {
    ...
    val minimumX = positionX min saveX
    val maximumX = positionX max saveX
    val minimumY = positionY min saveY
    val maximumY = positionY max saveY

    val xdiff = maximumX - minimumX + 1
    val ydiff = maximumY - minimumY + 1
    val area = xdiff * ydiff

    if (area < 9)
      return

    for {
      x <- minimumX to maximumX
      y <- minimumY to maximumY
      if field(x)(y) > 0
    } {
      score += field(x)(y)
      field(x)(y) = -1
