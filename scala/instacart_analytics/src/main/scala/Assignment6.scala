object Assignment6 {
  ...
  def evalExpr(e: Expr, env: Environment): Value = {
    ...
    e match {
      ...
      case Let(x, e1, e2) => {
        // YOUR CODE HERE
        val xe = evalExpr(e1, env)
        val updtaedEnv = env ++ Map(x -> xe)
        evalExpr(e2, updtaedEnv)
      }
      case MultiLet(xList, eList, e2) => {
        // YOUR CODE HERE
        val newMap = xList.zip(eList).map(x => {
          (x._1, evalExpr(x._2, env))
        }).toMap
        val updEnvMap = env ++ newMap
        evalExpr(e2, updEnvMap)
      }
    }
