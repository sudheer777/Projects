object CSAssignment6_2 {
  ...
  case class FunDefMulti(sList: List[String], e: Expr) extends Expr
  case class FunCallMulti(e1: Expr, e2List: List[Expr]) extends Expr
  ...
  def autoCurry(e: Expr): Expr = {
    e match {
      case FunDef(s, e) =>
        val eNew = autoCurry(e)
        FunDef(s, eNew)
      case FunCall(e1, e2) =>
        val e1New = autoCurry(e1)
        val e2New = autoCurry(e2)
        FunCall(e1New, e2New)
      case FunDefMulti(s, e1) =>
        if (s.isEmpty) {
          ...
          val t = s.tail
          val e1New = autoCurry(e1)
          val tailFuncMulti = autoCurry(FunDefMulti(t, e1New))
          FunDef(h, tailFuncMulti)
        }
      case FunCallMulti(e1, e2) =>
        if (e2.isEmpty) {
          ...
          val t = e2.tail
          val e1New = autoCurry(e1)
          val hNew = autoCurry(h)
          val funCallHead = FunCall(e1New, hNew)
          autoCurry(FunCallMulti(funCallHead, t))
        }
      case Let(s, e1, e2) =>
        val e1New = autoCurry(e1)
        val e2New = autoCurry(e2)
