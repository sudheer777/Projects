...
class DrawingCanvas extends Panel{
    ...
    override def paintComponent(g: Graphics2D) = {
       canvas match {
           case Some(c) => c.render(g, 600, 600)
           case _ => ()
        }
    }
...
object CanvasRenderTest extends SimpleSwingApplication {
    def top = new MainFrame {
        ...
        menuBar = new MenuBar {
            ...
            contents += new MenuItem( Action("StudentDefined") {
                execProgram("YourContribution", TestPrograms.studentDefined)
                textArea.text = TestPrograms.studentDefined()
            })
