...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double ) = {
        ...
        val xy = cList.map(_._2)
        (xs.min, xs.max, xy.min, xy.max)
    }
    ...
    override def translate(shiftX: Double, shiftY: Double): Polygon = {
        val ncList = cList.map(x => (x._1 + shiftX, x._2 + shiftY))
        Polygon(ncList)
    }

    def rotate(angRad: Double): Polygon = {
        val nl = cList.map(c => {
            (c._1 * math.cos(angRad) - c._2 * math.sin(angRad), c._1 * math.sin(angRad) + c._2 * math.cos(angRad))
        })
        Polygon(nl)
    }
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    def rotate(angRad: Double): MyCircle = {

        MyCircle((c._1 * math.cos(angRad) - c._2 * math.sin(angRad), c._1 * math.sin(angRad) + c._2 * math.cos(angRad)), r)
    }
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) = {
        val a = listOfObjects.map(_.getBoundingBox)
        (a.map(_._1).min, a.map(_._2).max, a.map(_._3).min, a.map(_._4).max)
    }
    ...
    def placeRight(myc2: MyCanvas):MyCanvas = {
        val b1 = getBoundingBox
        val b2 = myc2.getBoundingBox
        val mid1 = (b1._3 + b1._4) / 2
        val mid2 = (b2._3 + b2._4) / 2
        val n = myc2.translate(b1._2 - b2._1, mid1 - mid2)
        this.overlap(n)
    }
    ...
    def placeTop(myc2: MyCanvas): MyCanvas = {
        val b1 = getBoundingBox
        val b2 = myc2.getBoundingBox
        val mid1 = (b1._1 + b1._2) / 2
        val mid2 = (b2._1 + b2._2) / 2
        val n = myc2.translate(mid1 - mid2, b1._4 - b2._3)
        this.overlap(n)
    }
    ...
    def rotate(angRad: Double): MyCanvas = {
        val nl = listOfObjects.map {
            case c: MyCircle => c.rotate(angRad)
            case p: Polygon => p.rotate(angRad)
        }
        new MyCanvas(nl)
    }
